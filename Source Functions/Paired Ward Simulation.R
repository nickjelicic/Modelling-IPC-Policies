ward.simulation.paired <- function(focus.bay, infectious.states,
                            infection.rates, time.seq, 
                            infection.status, init.patient.attributes, infection.times,return.metrics,
                            restrictions=FALSE, quarantine.time=0, close.bay.coef=0, spread.to.focus.bay=FALSE, queue.departures=TRUE, 
                            new.patient.attributes, random.arrivals=TRUE){
  ## Initial Conditions
  #####
  patient.attributes <- init.patient.attributes[[1]]
  event.times <- init.patient.attributes[[2]]
  ...
  while(Finish==FALSE){
    ...
    if(event.time > end.time){
      ...
      if(event.time==first.arrival.time){
        ...
        empty.beds <- which(infection.status=="Empty")
        
        empty.beds.side.rooms <- empty.beds[which(empty.beds%%nrow(infection.status)==1)]
        empty.beds.focus.bay <- empty.beds[which(empty.beds%%nrow(infection.status)==2)]
        
        empty.beds.other.bays <- empty.beds[which(empty.beds%%nrow(infection.status)==0)]
        ...
        if(random.arrivals==TRUE){
          ...
          if(restrictions==TRUE){
            ...
            available.beds <- c(empty.beds.side.rooms, empty.beds.focus.bay, empty.beds.other.bays)
          }
        ...
        if(length(new.patient.location)>0){
          ...
          infection.status[new.patient.location] <- "Susceptible"
        ...
        infection.times <- infection.times.sample(T, infection.status, infection.rates, 2, infection.states, infectious.states, spread.to.focus.bay)
