ward.simulation.trial <- function(focus.bay, ward.size, infectious.states,arrival.rate, discharge.rates, departure.rates,
                            infection.rates, recovery.rates, time.seq, 
                            infection.status,
                            LOS.distribution, discharge.sd, departure.sd, scheduled.arrival.times=NULL, return.metrics,
  ...
  transfer.sd <- sqrt(departure.sd^2 + discharge.sd^2)
  ...
  arrived.times <- matrix(0, nrow=nrow(infection.status), ncol=ncol(infection.status))
  ...
  while(Finish==FALSE){
    ...
    if(event.time > end.time){
      ...
      metrics <- c(focus.bay.infections, other.bay.infections, new.arrivals.admitted, blocked.arrivals, unused_bed.days.focus.bay, unused_bed.days.other.bays, outbreak, infectious.discharged,
                   infectious.transferred, days.discharges.delayed, discharges, transfers)
      ...
      if(event.time==first.arrival.time){
        ...
        if(length(empty.beds.other.bays)>0){
          ...
          arrived.times[-c(1, focus.bay), ][new.patient.location] <- T
          ...
          arrived.times[focus.bay, new.patient.location] <- T
          ...
          arrived.times[1, new.patient.location] <- T
      ...
      if(event.time==first.discharge.time){
        ...
        LOS <- c(LOS, T - arrived.times[discharge])
        arrived.times[discharge] <- 0
      ...
      if(event.time==first.transfer.time){
        ...
        if(transfer%%n.bay==focus.bay & restrictions==TRUE){
          ...
          LOS <- c(LOS, T - arrived.times[transfer])
          arrived.times[transfer] <- 0
        }
  ...
  if(return.metrics==TRUE){
    return(c(LOS, rep(time.seq[length(time.seq)], length(which(infection.status%in%infection.states[-1] & arrived.times==0)))))
  }else{
