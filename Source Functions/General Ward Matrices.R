general.ward.matrices <- function(bay.size, ward.size, infection.states, infectious.states, outbreak, random.arrivals=TRUE){
  ...
  Eta.1.general <- matrix(0, nrow=n, ncol=n)
  Eta.2.general <- matrix(0, nrow=n, ncol=n)
  Eta.3.general <- matrix(0, nrow=n, ncol=n)
  Outbreak.general <- matrix(0, nrow=n, ncol=n)
    ...
    for(j in 1:n){
      ...
      if(diff == 1){
        ...
        if(any(c(state.1, state.2)==0)){
          if(state.1==0){
            Outbreak.general[i,j] <- 1
            
            if(length(which(states.lab[i, ]==infectious.states[1]))>0){
              Eta.1.general[i,j] <- infection.string[1, base.states*length(which(states.lab[i, ]==infectious.states[1]))]
            ...
            if(length(infectious.states)>1){
              if(length(which(states.lab[i, ]==infectious.states[2]))>0){
                ...
                Eta.2.general[i,j] <- infection.string[2, base.states*length(which(states.lab[i, ]==infectious.states[2]))]
            ...
            if(length(infectious.states)>2){
              if(length(which(states.lab[i, ]==infectious.states[3]))>0){
                ...
                Eta.3.general[i,j] <- infection.string[3, base.states*length(which(states.lab[i, ]==infectious.states[3]))]
              }
            }
          ...
          if(state.2==state.1+1){
            Q[i,j] <- string.matrix[1, 1]
          }else if(state.1==state.2 + 1){
            Q[i,j] <- string.matrix[10, as.numeric(states.lab[i, ncol(states.lab)])]
          }
          ...
          if(rates[state.1, state.2]=="lambda"){
            ...
            Q[i,j] <- string.matrix[2, base.states]
            
          }else if(rates[state.1, state.2]=="mu.2"){
            
            Q[i,j] <- string.matrix[3, base.states]
            
          }else if(rates[state.1, state.2]=="eta.background"){
            
            Q[i,j] <- string.matrix[4, base.states]
          }else if(rates[state.1, state.2]=="rho"){
            
            Q[i,j] <- string.matrix[5, base.states]
          }else if(rates[state.1, state.2]=="gamma.1"){
            
            Q[i,j] <- string.matrix[6, base.states]
          }else if(rates[state.1, state.2]=="gamma.2"){
            
            Q[i,j] <- string.matrix[7, base.states]
          }else if(rates[state.1, state.2]=="gamma.3"){
            
            Q[i,j] <- string.matrix[8, base.states]
          }
  ...
  return(list(Q, Eta.1.general, Eta.2.general, Eta.3.general, Outbreak.general))
